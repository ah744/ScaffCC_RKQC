...
@names = split (/\./, $file);
#$name = $names[0];
$scaf = "$names[0]\_norevkit.scaffold";
...
while (<SCAF>) {
	if ( /^extern void\s+$module/ ) {
		...
		print MQHF;
		last;				# Process revkit output and insert first; we'll fill in the rest of the file later
	} else {
...
#	next	if (/qubit [^I]/); # Skip non-ancilla qbit definitions (ancilla start with I)
	if (/qubit (\w+)I(\d+)$/){ # Skip non-ancilla qbit definitions (ancilla start with I)
        $workers{$1} = $2 + 1;
        next;
    }
...
# Reinsert qubit defs
foreach $work ( keys %workers ) {
    splice @defn, 0, 0, "   qbit $work\[$workers{$work}\];\n";
}
