...
/*--------- Misc Testing File for SHA-1 Helpers -------------*/
#include <iostream>
...
revkit nToffoli(qint target, qint in1, qint in2){
...
revkit AND(qint target, qint in1, qint in2){
    nToffoli(target, in1, in2);
}

revkit NAND(qint target, qint in1, qint in2){
    int i;
    AND(target, in1, in2);
    for(i=0; i<32; i++){
        NOT(target[i]);
...
revkit NOR(qint target, qint in1, qint in2){
    int i;
...
revkit OR(qint target, qint in1, qint in2){
    int i;
    NOR(target, in1, in2);
    for(i=0; i<32; i++){
        NOT(target[i]);
...
revkit XOR(qint target, qint in1, qint in2){
    zero_to_garbage temp1(32);
    zero_to_garbage temp2(32);
...
revkit reverseBits(qint a, int n){
...
revkit pad(qint a, qint input, qint padding){
...
revkit map(qint a, qint in, int n){
    for(int i=0; i<32; i++)
    {
...
revkit finalHash(qint a, qint h0, qint h1, qint h2, qint h3, qint h4){
    int i;
    for(i=0; i<32; i++)
    {
...
revkit encrypt( qbit in, qbit ha ){
