...
namespace revkit
{
  ...
  void weighted_reordering::operator()( cubes_type& cubes ) const
  {
    ...
    std::copy( boost::make_counting_iterator( 0u ), boost::make_counting_iterator( (unsigned)vars.size() ), vars.begin() );
    ...
    if ( separate_polarities )
    {
      ...
      for ( binary_truth_table::const_iterator it = spec.begin(); it != spec.end(); ++it )
      {
        ...
        foreach ( const binary_truth_table::value_type& in_bit, it->first )
        {
          if ( in_bit )
          {
            controls += ( 1u - *in_bit ) * n + index; // considers polarity to choose line
          }
          ++index;
        }
  ...
  {
    pla_blif_synthesis_func f = boost::bind( esop_synthesis, _1, _2, settings, statistics );
