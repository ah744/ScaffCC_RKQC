...
namespace revkit
{
  ...
  void get_control_lines_from_mask( unsigned mask, unsigned bw, gate::line_container& controls )
  {
  ...
  void basic_first_step( circuit& circ, std::vector<unsigned>& output_values )
  {
    ...
    for ( unsigned i = 0; i < bw; ++i )
    {
      ...
      {
        prepend_not( circ, i );
        foreach ( unsigned& ov, output_values )
        {
          ov ^= ( 1u << num_pos );
        }
      }
  ...
  truth_table_synthesis_func transformation_based_synthesis_func( properties::ptr settings,
                                                                  properties::ptr statistics )
  {
    truth_table_synthesis_func f = boost::bind( transformation_based_synthesis, _1, _2, settings, statistics );
