...
namespace revkit
{
  ...
  unsigned additional_garbage( const binary_truth_table& base, std::vector<unsigned>& values )
  {
    ...
    unsigned mu = std::max_element( output_value_count.begin(), output_value_count.end(),
                                    boost::bind( std::less<unsigned>(),
                                                 boost::bind( &std::map<unsigned,unsigned>::value_type::second, _1 ),
                                                 boost::bind( &std::map<unsigned,unsigned>::value_type::second, _2 ) ) )->second;

    std::transform( output_value_count.begin(), output_value_count.end(), std::back_inserter( values ), boost::bind( &std::map<unsigned,unsigned>::value_type::first, _1 ) );
  ...
  {
    embedding_func f = boost::bind( embed_truth_table, _1, _2, settings, statistics );
